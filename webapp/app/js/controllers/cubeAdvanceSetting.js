...
KylinApp.controller('CubeAdvanceSettingCtrl', function ($scope, $modal,cubeConfig,MetaModel,TableService,cubesManager,CubeDescModel,SweetAlert,VdmUtil,modelsManager) {
  ...
  $scope.checkColumn = function (){
    ...
    if(isColumnExit){
      SweetAlert.swal('提示...', "The column named [" + $scope.newDictionaries.column + "] already exists", 'warning');
      return false;
    }
    return true;
  }
  ...
  $scope.uploadMandatoryDimensionSetList = function() {
    ...
    if (file) {
      ...
      swal('提示...', 'Please choose your file first.', 'warning');
    }
  };
  ...
  $scope.addDimensionSet = function() {
    if ($scope.mandatoryDimensionSet.select.length) {
      ...
      if (!existed) {
        ...
        swal('提示...', 'Dimension set already existed', 'warning');
      }
    } else {
      swal('提示...', 'Dimension set should not be empty', 'warning');
    }
  };
  ...
  $scope.addSnapshot = function(newSnapshot) {
    ...
    if (!newSnapshot.table_name || !newSnapshot.storage_type) {
      swal('提示...', 'Snapshot table name or storage should not be empty', 'warning');
      return;
      ...
      if (!!existSnapshot) {
        swal('提示...', 'Snapshot table already existed', 'warning');
        return;
      }
    }
